...
class LuaSync {
   ...
   */
  constructor(opts = {}) {
    // ★ Obfuscated cache dir name (.cache thay vì luas) — không reveal purpose
    this.cacheDir = opts.cacheDir || path.join(process.env.APPDATA || '.', 'hp-action-live', '.cache');
  ...
  _ensureCacheDir() {
    try {
      if (!fs.existsSync(this.cacheDir)) {
        ...
        // ★ Hide folder ngay sau khi tạo (Hidden + System attribs trên Windows)
        this._hideCacheFolder();
      }
    } catch (e) {
  ...
  // ★ Apply Windows Hidden + System attributes → không hiện trong Explorer mặc định
  _hideCacheFolder() {
  ...
  // ★ Obfuscate luaId thành filename hash (deterministic per install)
  // VD: luaId="mario" + installId="xxx" → "a8f3b2c1.lua"
  // Cùng installId → cùng hash (stable). Khác máy → khác hash (no leak).
  _obfuscatedName(luaId) {
    if (!this.installId) this._ensureInstallId();
    const h = crypto.createHash('sha256')
      .update((this.installId || '') + ':' + String(luaId))
      .digest('hex');
    return h.slice(0, 10);   // 10 hex chars = đủ collision-free cho 100 LUAs
  }
  ...
  async downloadLua(luaId) {
    ...
    // ★ STRIP COMMENTS + obfuscate print prefixes (chống lộ effect name trong file)
    // 1. Bỏ block comments `--[[ ... ]]`
    content = content.replace(/--\[\[[\s\S]*?\]\]/g, '');
    // 2. Bỏ tất cả single-line comments `-- ...` (kể cả header HP Media)
    content = content.replace(/^[ \t]*--[^\r\n]*$/gm, '');
    // 3. Collapse blank lines
    content = content.replace(/\n[ \t]*\n[ \t]*\n+/g, '\n\n');
    // 4. Obfuscate print prefix `[lua_id]` thành `[fx-<obName>]`
    //    VD: print("[lo_xo] LỖI: ...") → print("[fx-6eba6b9d67] LỖI: ...")
    ...
    this.logger.log(`✓ Downloaded ${safeId} → ${obName}.lua v${entry.version} (stripped: ${buf.length}→${finalBuf.length} bytes)`);
    this._emit('downloaded', { id: safeId, version: entry.version, size: finalBuf.length, path: filePath });

    // ★ Generate decoy files để đánh lạc hướng (chỉ tạo lần đầu, không tạo lại mỗi download)
