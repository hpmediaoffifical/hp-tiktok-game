...
class LuaSync {
   ...
   */
  constructor(opts = {}) {
    this.cacheDir = opts.cacheDir || path.join(process.env.APPDATA || '.', 'hp-action-live', 'luas');
  ...
  _ensureCacheDir() {
    try {
      if (!fs.existsSync(this.cacheDir)) {
        ...
        this.logger.log(`Created cache dir: ${this.cacheDir}`);
      }
    } catch (e) {
  ...
  getLocalState(luaId) {
    ...
    const filePath = path.join(this.cacheDir, `${safeId}.lua`);
    const metaPath = path.join(this.cacheDir, `${safeId}.meta.json`);
    ...
    return { id: safeId, exists, version, downloadedAt, size, path: filePath };
  }
  ...
  async downloadLua(luaId) {
    ...
    this.logger.log(`✓ Downloaded ${safeId} v${entry.version} (${buf.length} bytes, sha256=${hash.slice(0,12)}...)`);
    this._emit('downloaded', { id: safeId, version: entry.version, size: buf.length, path: filePath });
    return { ok: true, id: safeId, version: entry.version, path: filePath, size: buf.length, hash: hash.slice(0, 16) };
  }
  ...
  // ===== Full state for API/UI (URL HIDDEN!) =====
  getFullState() {
    const luas = (this.cachedManifest?.luas || []).map(entry => {
      ...
      return {
        ...
        // url: HIDDEN — không expose ra UI
        status: this.getStatus(entry.id),
        localVersion: local.version,
        downloadedAt: local.downloadedAt,
        localSize: local.size,
        localPath: local.path     // safe to expose (user cần để add vào OBS Scripts)
      };
    });
    return {
