# publish-v1.1.2.ps1 — Upload + publish v1.1.2 lên license.hpvn.media
# CÁCH DÙNG:
...
$VERSION  = "1.1.2"
$FILENAME = "HP-Action-LIVE-Setup-1.1.2.exe"
...
$notes = @"
v1.1.2 — Hotfix: panel position sliders not syncing to OBS

BUG (gap o v1.1.0+):
- Slider Vi tri / Scale cua Bieu Cam Nhiet va Ban Cung trong panel
  chinh xong khong sync ra OBS. Chi sua duoc trong overlay edit mode.

NGUYEN NHAN:
- v1.1.0 them server filter de fix race condition drag-revert. Filter
  block TAT CA drag fields tu POST non-popout (= panel) -> panel slider
  thay doi cfg local, POST den server bi server bo qua -> OBS khong sync.

FIX:
- Bo filter. Server accept all POST nhu cu (whoever posts last wins).
- Race condition drag-revert it khi xay ra trong thuc te (user thuong
  doi 200ms+ sau drag); neu xay ra co the keo lai.
"@
...
Write-Host ""
Write-Host "DONE — v1.1.2 da san sang. User mo app cu se thay modal cap nhat." -ForegroundColor Magenta
