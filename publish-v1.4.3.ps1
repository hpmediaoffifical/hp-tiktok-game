# publish-v1.4.3.ps1 — Upload + publish v1.4.3 lên license.hpvn.media
# CÁCH DÙNG:
...
$VERSION  = "1.4.3"
$FILENAME = "HP-Action-LIVE-Setup-1.4.3.exe"
...
$notes = @"
v1.4.3 — Fix giong doc Dich Thuat LIVE bi 2 tieng de nhau tren OBS

FIX:
- Dich Thuat LIVE: khi dat cung 1 link overlay vao NHIEU Scene OBS, moi
  Browser Source la 1 trinh duyet rieng, deu phat TTS -> 2-3 tieng de nhau.
  Server gio bau "speaker" duy nhat (leader-election) -> chi 1 overlay doc,
  cac overlay con lai chi hien chu. Speaker dong -> tu bau cai ke tiep.
- Fix de tieng "lau lau" voi cau DAI: nhanh Google TTS dung watchdog cung
  12s, clip dai hon 12s -> bi cat som va cau ke phat de len clip cu. Gio
  watchdog tinh theo do dai that cua clip + luon dung (pause) clip cu truoc
  khi sang cau ke. Doc tuan tu triet de.
"@
...
Write-Host ""
Write-Host "DONE — v1.4.3 da san sang. User mo app cu se thay modal cap nhat." -ForegroundColor Magenta
